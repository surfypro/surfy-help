...
For this, you must first create the usage types to use:

From the left menu,
-   click on "Furniture" then "Typologies" then "Workstation usage type"
-   to list existing usage types, click on "List workstation usage types". Each usage type can be modified by clicking on the icon at the beginning of the usage type line to "Modify workstation usage type XXX".
-   to create a new workstation usage type, click on "Create a workstation usage type". Fill in the name and color associated with the usage type then validate the creation.

Once the usage types are created, you can associate each workstation with a usage type and visualize workstations on the plan, in the color of their usage type.

To associate a usage type to a workstation,

-   click on the workstation to qualify
-   "Modify" the workstation in the workstation card appearing on the left of the plan
-   click on the "Pencil" icon of the "workstation usage type"
-   select the desired usage type for the workstation
-   save the modification

To visualize workstations according to their usage type,

-   display the floor plan to visualize
-   click on the vertical "label" icon to "color workstations by their usage type"
-   click on "select all" to color workstations on the plan according to their usage type.

 You can color all or part of the usage types by clicking on the checkboxes of each usage type.

The number of workstations per usage type is indicated opposite each usage type.
